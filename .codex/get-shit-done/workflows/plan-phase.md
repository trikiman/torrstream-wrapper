...
```bash
INIT=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" init plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-researcher 2>/dev/null)
AGENT_SKILLS_PLANNER=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-planner 2>/dev/null)
AGENT_SKILLS_CHECKER=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-checker 2>/dev/null)
...
```bash
PHASE_INFO=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
DISCUSS_MODE=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.discuss_mode 2>/dev/null || echo "discuss")
```
...
```bash
PHASE_DESC=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section)
```
...
**If found:**
1. Read template: `E:/Projects/torrserver/.codex/get-shit-done/templates/VALIDATION.md`
...
SECURITY_ASVS=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```bash
UI_PHASE_CFG=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase 2>/dev/null || echo "true")
UI_GATE_CFG=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" 2>/dev/null)
...
```bash
PHASE_SECTION=$(node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
2. Disable Nyquist with the exact command:
   `node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" config-set workflow.nyquist_validation false`
...
```bash
node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" state planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
if ([[ "{{GSD_ARGS}}" =~ --auto ]] || [[ "{{GSD_ARGS}}" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  node "E:/Projects/torrserver/.codex/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active true
fi
```
